...
SELECT first_name, last_name, COUNT(owners.id) 
  FROM owners RIGHT JOIN vehicles ON owners.id = vehicles.owner_id 
  GROUP BY owners.id
  ORDER BY first_name;
...
SELECT first_name, last_name, ROUND( AVG(price)::numeric, 0 ) as average_price, COUNT(owners.id)
  FROM owners RIGHT JOIN vehicles ON owners.id = vehicles.owner_id 
  GROUP BY owners.id
  HAVING COUNT(owners.id) > 1 
  AND AVG(price) > 10000
